...
export default function HouseForm({ house }: IProps) {
  ...
  const [previewImage, setPrieviewImage] = useState("");
  ...
  const handleCreate = async (data: IFormData) => {
    ...
    if (signatureData) {
      ...
      if (houseData?.createHouse) {
        router.push(`${houseData.createHouse.id}`);
      }
    }
  };
            ...
            <input
              ...
              onChange={(event: ChangeEvent<HTMLInputElement>) => {
                if (event?.target?.files?.[0]) {
                  ...
                  reader.onloadend = () => {
                    setPrieviewImage(reader.result as string);
                  };
          ...
          <div className="mt-4">
            ...
            <input
              ...
              ref={register({
                ...
                max: { value: 10, message: "Woah too big" },
                min: { value: 1, message: "Must have 1 bedroom" },
              })}
            />
